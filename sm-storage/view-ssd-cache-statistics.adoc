...
Les statistiques nominales, qui constituent un sous-ensemble des statistiques détaillées, sont affichées dans la boîte de dialogue Afficher les statistiques du cache du disque SSD. Vous pouvez afficher les statistiques détaillées du cache SSD uniquement lorsque vous exportez toutes les statistiques SSD vers un `.csv` fichier.
