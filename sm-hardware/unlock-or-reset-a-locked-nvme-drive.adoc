...
.Avant de commencer
* Pour l'option déverrouiller, assurez-vous que le fichier de clé de sécurité (avec l'extension `.slk` ) est disponible sur le client de gestion (le système avec un navigateur utilisé pour accéder à System Manager). Vous devez également connaître la phrase de passe associée à la clé.
