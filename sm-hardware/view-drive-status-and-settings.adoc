...
Affecté à/peut protéger
 a| 
Si le disque est affecté à un pool, à un groupe de volumes ou à un cache SSD, ce champ affiche « attribué à ». La valeur peut être un nom de pool, un nom de groupe de volumes ou un nom de cache SSD. Si le disque est affecté à un disque de secours et que son mode est Veille, ce champ affiche « peut protéger pour ». Si le disque de secours peut protéger un ou plusieurs groupes de volumes, les noms des groupes de volumes apparaissent. S'il ne peut pas protéger un groupe de volumes, il affiche 0 groupes de volumes.

Si le lecteur est affecté à un disque de secours et que son mode est utilisé, ce champ affiche « protection ». La valeur est le nom du groupe de volumes affecté.
